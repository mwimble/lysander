...
void WRDifferentialDrive::init() {
	...
	for (std::size_t jointId = 0; jointId < jointNames_.size(); ++jointId) {
		...
		jointStateInterface_.registerHandle(hardware_interface::JointStateHandle(
			jointNames_[jointId],
		...
		hardware_interface::JointHandle jointHandlePosition = hardware_interface::JointHandle(
			jointStateInterface_.getHandle(jointNames_[jointId]),
...
void WRDifferentialDrive::registerJointLimits(const hardware_interface::JointHandle &jointHandlePosition,
	...
	if (model_ == NULL) {
		ROS_WARN_STREAM("[WRDifferentialDrive::registerJointLimits] No URDF model loaded, unable to get joint limits");
		return;
	}
	...
	if (joint_limits_interface::getJointLimits(urdfJoint, jointLimits)) {
		hasJointLimits = true;
		ROS_ERROR_STREAM("[WRDifferentialDrive::registerJointLimits] Joint " 
						 << jointNames_[jointId] 
	...
	if (hasSoftLimits) {  // Use soft limits
		ROS_ERROR_STREAM("[WRDifferentialDrive::registerJointLimits] Using soft saturation limits");
		...
		ROS_ERROR_STREAM("[WRDifferentialDrive::registerJointLimits] Using saturation limits (not soft limits)");
